...
namespace Drupal\Tests\druki\Unit\Repository;
...
final class EntitySyncQueueStateTest extends UnitTestCase {
  ...
  protected function setUp(): void {
    ...
    $state->get(Argument::type('string'), Argument::any())->will(static function ($args) use ($self) {
      $storage_key = $args[0];
    ...
    $state->set(Argument::type('string'), Argument::any())->will(static function ($args) use ($self): void {
      $storage_key = $args[0];
      $value = $args[1];
      $self->stateStorage[$storage_key] = $value;
    });
    $state->delete(Argument::type('string'))->will(static function ($args) use ($self): void {
      $storage_key = $args[0];
