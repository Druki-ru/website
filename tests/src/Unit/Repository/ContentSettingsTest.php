...
final class ContentSettingsTest extends UnitTestCase {
   ...
   *
   * @return \Drupal\Core\KeyValueStore\KeyValueFactoryInterface The config factory instance.
  ...
  protected function getKeyValueFactory(): KeyValueFactoryInterface {
    ...
    $content_settings->set('content_source_uri', Argument::any())->will(function ($args) use ($content_settings) {
    ...
    $content_settings->set('repository_url', Argument::any())->will(function ($args) use ($content_settings) {
      $content_settings->get('repository_url')->willReturn($args[1]);
      return $content_settings->reveal();
    });

    $content_settings->get('webhook_url')->willReturn('default');
    $content_settings->set('webhook_url', Argument::any())->will(function ($args) use ($content_settings) {
